...
// Create diagonal timeline line
const line = d3.line()
  .x((d, i) => xScale(d.startYear + (d.endYear - d.startYear) / 2))
  .y((d, i) => yScale(i))
...
const journeyItems = g.selectAll(".journey-item")
  ...
  .attr("transform", (d, i) => `translate(${xScale(d.startYear)}, ${yScale(i)})`);

// Add timeline bars (horizontal)
...
// Add connecting dots
journeyItems.append("circle")
  .attr("cx", d => (xScale(d.endYear) - xScale(d.startYear)) / 2)
  .attr("cy", 0)
  .attr("r", 6)
  .style("fill", "white")
  .style("stroke", d => d.type === "education" ? "#4facfe" : "#43e97b")
  .style("stroke-width", 3);

// Add title text (positioned to the right of bars)
journeyItems.append("text")
  .attr("x", d => (xScale(d.endYear) - xScale(d.startYear)) + 15)
  .attr("y", -5)
  .style("fill", "white")
  .style("font-size", "14px")
...
journeyItems.append("text")
  .attr("x", d => (xScale(d.endYear) - xScale(d.startYear)) / 2)
  .attr("y", 4)
