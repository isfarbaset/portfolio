---
title: "About Me"
image: images/ib-new.png
page-layout: full
output-dir: ../../../docs
format:
  html:
    css: style.css
about:
  template: jolla
  image-shape: round
  image-width: 15em
  links:
    - icon: linkedin
      text: LinkedIn
      href: https://www.linkedin.com/in/isfarbaset/
    - icon: github
      text: Github
      href: https://github.com/isfarbaset
    - icon: instagram
      text: Instagram
      href: https://www.instagram.com/is.far/
---
            newLayout = {
                height: 450,
                width: null, // Remove width constraint for desktop
                margin: {l: 200, r: 400, t: 30, b: 70},
                'xaxis.range': [-0.8, 1.8]
            };
        }w.linkedin.com/in/isfarbaset/
    - icon: github
      text: Github
      href: https://github.com/isfarbaset
    - icon: instagram
      text: Instagram
      href: https://www.instagram.com/is.far/
---

::: {.about-content}

::: {.about-intro}
Hello! I'm originally from **Dhaka**, the vibrant capital city of Bangladesh. I came to the US in 2015 in pursuit of my bachelor's degree in Computer Science and lived in West Michigan before moving to Northern Virginia in 2022. 

I am currently a **Data Analyst II** at Shift Digital in the Data Operations and Analytics team, where I leverage data-driven solutions for enterprise clients. I graduated with a **Master's of Science in Data Science & Analytics** from Georgetown University in 2025. My passion lies in leveraging tailored data and algorithms to solve intricate business problems and communicating complex insights to non-technical stakeholders.

I love spending time in charming cafes and exploring new cuisines. During my downtime I enjoy reading, painting and watching movies/TV shows.
:::

::: {.section-card}
## 📊 Professional Journey

::: {.journey-visual}
```{python}
#| echo: false
import plotly.graph_objects as go

# Timeline data organized chronologically
timeline_data = [
    {
        'year': '2015-2020',
        'title': 'Bachelor of Science in Computer Science',
        'organization': 'Grand Valley State University',
        'location': 'Grand Rapids, MI',
        'type': 'education',
        'description': 'Foundation in computer science and programming'
    },
    {
        'year': '2020-2021',
        'title': 'Computer Engineer',
        'organization': 'Array of Engineers',
        'location': 'Grand Rapids, MI',
        'type': 'work',
        'description': 'Aerospace software verification and web development'
    },
    {
        'year': '2021-2023',
        'title': 'Data Analyst I',
        'organization': 'Shift Digital',
        'location': 'Birmingham, MI',
        'type': 'work',
        'description': 'Data operations and analytics for enterprise clients'
    },
    {
        'year': '2023-2025',
        'title': 'Master of Science in Data Science and Analytics',
        'organization': 'Georgetown University',
        'location': 'Washington, D.C.',
        'type': 'education',
        'description': 'Advanced data science and analytics specialization'
    },
    {
        'year': '2025-Present',
        'title': 'Data Analyst II',
        'organization': 'Shift Digital',
        'location': 'Birmingham, MI',
        'type': 'work',
        'description': 'Leading analytics initiatives and mentoring team members'
    }
]

fig = go.Figure()

# Define colors
colors = {
    'education': '#4A90E2',  # Professional blue
    'work': '#7ED321'       # Fresh green
}

# Create vertical timeline
for i, item in enumerate(timeline_data):
    y_pos = len(timeline_data) - i - 1  # Reverse order (newest at top)
    color = colors[item['type']]
    
    # Timeline dot with custom hover styling
    fig.add_trace(go.Scatter(
        x=[0],
        y=[y_pos],
        mode='markers',
        marker=dict(
            size=16,
            color=color,
            line=dict(width=3, color='white'),
            symbol='circle'
        ),
        hovertemplate=(
            f"<b style='color:white'>{item['title']}</b><br>"
            f"<span style='color:white'>{item['organization']}</span><br>"
            f"<span style='color:white'>{item['location']}</span><br>"
            f"<span style='color:white'>{item['year']}</span><br>"
            f"<i style='color:white'>{item['description']}</i>"
            "<extra></extra>"
        ),
        hoverlabel=dict(
            bgcolor=color,
            font=dict(color='white', size=12, family='Inter, Arial, sans-serif'),
            bordercolor='white'
        ),
        showlegend=False
    ))
    
    # Title and organization (right side) - much more spacing from timeline
    fig.add_annotation(
        x=0.3,
        y=y_pos + 0.15,
        text=f"<b>{item['title']}</b>",
        showarrow=False,
        font=dict(size=14, color='#2C3E50', family='Inter, Arial, sans-serif'),
        xanchor='left',
        yanchor='middle',
        name='title-text'
    )
    
    fig.add_annotation(
        x=0.3,
        y=y_pos - 0.05,
        text=f"{item['organization']}",
        showarrow=False,
        font=dict(size=12, color='#5D6D7E', family='Inter, Arial, sans-serif'),
        xanchor='left',
        yanchor='middle',
        name='org-text'
    )
    
    fig.add_annotation(
        x=0.3,
        y=y_pos - 0.25,
        text=f"{item['location']}",
        showarrow=False,
        font=dict(size=10, color='#85929E', family='Inter, Arial, sans-serif'),
        xanchor='left',
        yanchor='middle',
        name='location-text'
    )
    
    # Year (left side) - more spacing from timeline
    fig.add_annotation(
        x=-0.3,
        y=y_pos,
        text=f"<b>{item['year']}</b>",
        showarrow=False,
        font=dict(size=11, color='#34495E', family='Inter, Arial, sans-serif'),
        xanchor='right',
        yanchor='middle',
        name='year-text'
    )

# Vertical timeline line
fig.add_trace(go.Scatter(
    x=[0, 0],
    y=[-0.5, len(timeline_data) - 0.5],
    mode='lines',
    line=dict(color='#D5DBDB', width=3),
    showlegend=False,
    hoverinfo='skip'
))

# Legend
fig.add_trace(go.Scatter(
    x=[None], y=[None],
    mode='markers',
    marker=dict(size=12, color=colors['education']),
    name='Education',
    showlegend=True
))

fig.add_trace(go.Scatter(
    x=[None], y=[None],
    mode='markers',
    marker=dict(size=12, color=colors['work']),
    name='Professional Experience',
    showlegend=True
))

# Layout with desktop defaults - much more generous spacing
fig.update_layout(
    showlegend=True,
    legend=dict(
        orientation="h",
        yanchor="bottom",
        y=-0.15,
        xanchor="center",
        x=0.5,
        font=dict(size=11, family='Inter, Arial, sans-serif')
    ),
    plot_bgcolor='rgba(0,0,0,0)',
    paper_bgcolor='rgba(0,0,0,0)',
    height=450,
    margin=dict(l=200, r=400, t=30, b=70),
    xaxis=dict(
        showgrid=False,
        showticklabels=False,
        zeroline=False,
        range=[-0.8, 1.8]
    ),
    yaxis=dict(
        showgrid=False,
        showticklabels=False,
        zeroline=False,
        range=[-0.5, len(timeline_data) - 0.5]
    )
)

# Configure for responsive display
config = {
    'responsive': True,
    'displayModeBar': False,
    'showTips': False
}

fig.show(config=config)
```

::: {#timeline-responsive-script}
<script>
// Temporarily disabled to test if this is overriding the generous desktop layout
/*
document.addEventListener('DOMContentLoaded', function() {
    function adjustTimelineLayout() {
        const timelineContainer = document.querySelector('.journey-visual .plotly-graph-div');
        if (!timelineContainer) return;
        
        const screenWidth = window.innerWidth;
        let newLayout = {};
        
        if (screenWidth <= 768) {
            // Mobile: Fixed width with compact margins for horizontal scrolling
            newLayout = {
                height: 350,
                width: 600, // Fixed width enables horizontal scroll
                margin: {l: 80, r: 150, t: 20, b: 60},
                'xaxis.range': [-0.4, 1.0]
            };
        } else {
            // Desktop: Much more generous spacing to prevent scrunched appearance
            newLayout = {
                height: 450,
                width: null, // Remove width constraint for desktop
                margin: {l: 200, r: 400, t: 30, b: 70},
                'xaxis.range': [-0.8, 1.8]
            };
        }
        
        // Apply the layout changes
        if (window.Plotly && timelineContainer._fullLayout) {
            Plotly.relayout(timelineContainer, newLayout);
        }
    }
    
    // Initial adjustment with a delay for DOM readiness
    setTimeout(adjustTimelineLayout, 500);
    
    // Debounced resize handler
    let resizeTimeout;
    function handleResize() {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(adjustTimelineLayout, 150);
    }
    
    // Event listeners
    window.addEventListener('resize', handleResize);
    window.addEventListener('orientationchange', function() {
        setTimeout(adjustTimelineLayout, 300);
    });
::: {#timeline-responsive-script}
<script>
// Fixed JavaScript - only applies layout changes to mobile, leaves desktop defaults alone
document.addEventListener('DOMContentLoaded', function() {
    function adjustTimelineLayout() {
        const timelineContainer = document.querySelector('.journey-visual .plotly-graph-div');
        if (!timelineContainer) return;
        
        const screenWidth = window.innerWidth;
        
        // Only apply layout changes for mobile - let desktop use the generous Python defaults
        if (screenWidth <= 768) {
            const newLayout = {
                height: 350,
                width: 600, // Fixed width enables horizontal scroll
                margin: {l: 80, r: 150, t: 20, b: 60},
                'xaxis.range': [-0.4, 1.0]
            };
            
            // Apply mobile layout changes
            if (window.Plotly && timelineContainer._fullLayout) {
                Plotly.relayout(timelineContainer, newLayout);
            }
        }
        // Desktop: Do nothing - keep the generous Python layout defaults
    }
    
    // Initial adjustment with a small delay for DOM readiness  
    setTimeout(adjustTimelineLayout, 300);
    
    // Debounced resize handler
    let resizeTimeout;
    function handleResize() {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(adjustTimelineLayout, 150);
    }
    
    // Universal event listeners
    window.addEventListener('resize', handleResize);
    window.addEventListener('orientationchange', function() {
        setTimeout(adjustTimelineLayout, 300);
    });
});
</script>
:::
:::

::: {.resume-download}
For a detailed overview of my experience:  
[Download Resume](Isfar Baset Resume.pdf){.btn .btn-primary .download-btn}
:::
:::

::: {.section-card}
## 🎓 Education

::: {.education-item}
### Georgetown University
::: {.edu-details}
**Master of Science in Data Science and Analytics** • GPA: 4.0  
📍 Washington, D.C. • May 2023 - May 2025
:::

::: {.coursework}
**Key Coursework:**  
Probabilistic Modeling • Database Systems • Advanced Data Visualization • Data Ethics • Computational Linguistics • Machine Learning Deployment • Digital Storytelling • Big Data & Cloud Computing • Applied Generative AI
:::

::: {.activities}
**Leadership:** Social Committee Member & Lead Mentor in the Data Science and Analytics program
:::
:::

::: {.education-item}
### Grand Valley State University
::: {.edu-details}
**Bachelor of Science in Computer Science**  
📍 Grand Rapids, MI • Aug 2015 - May 2020
:::
:::
:::

::: {.section-card}
## 💼 Professional Experience

::: {.experience-item}
### Data Analyst II (Current)
::: {.company-info}
**Shift Digital, Data Operations and Analytics Team** • Birmingham, MI • May 2025 - Present
:::

::: {.achievements}
- Leading advanced data operations and analytics initiatives for enterprise clients
- Driving strategic data architecture decisions and mentoring junior analysts
:::
:::

::: {.experience-item}
### Data Analyst I
::: {.company-info}
**Shift Digital, Data Operations and Analytics Team** • Birmingham, MI • Nov 2021 - May 2023
:::

::: {.achievements}
- Improved data accuracy by **25%** across 10+ enterprise projects by optimizing big data ETL processes
- Worked with cross-functional teams to design scalable data architecture, delivering actionable findings to senior executives
- Streamlined operations for 12 high-value clients and saved **15 hours weekly** by automating billing data processing
- Managed data migration and transformation within Databricks, creating efficient pipelines for business-critical insights
- Leveraged SQL and Python within PySpark to transform raw data into cleaned, analysis-ready datasets
- Enhanced data visibility with interactive dashboards, enabling faster decisions for key stakeholders
- Crafted technical proposals achieving **near 100% customer approval rate**
:::
:::

::: {.experience-item}
### Computer Engineer
::: {.company-info}
**Array of Engineers** • Grand Rapids, MI • Jul 2020 - Nov 2021
:::

::: {.achievements}
- Verified Airbus A350 avionics software using DOORS and Ansys SCADE, ensuring aerospace industry compliance
- Improved user experience by optimizing web app features with AngularJS, increasing client retention by **25%**
- Introduced modular workflows for software testing, achieving **98% on-time delivery**
- Conducted A/B testing on web app features, identifying improvements to enhance functionality and usability
- Led GitHub code reviews across multidisciplinary teams, mitigating critical issues early in development
:::
:::
:::

::: {.section-card}
## 🛠️ Technical Skills

::: {.skills-grid}
::: {.skill-category}
**Programming**  
Python • SQL • R • Java • C++
:::

::: {.skill-category}
**Machine Learning**  
Scikit-learn • TensorFlow • PyTorch • XGBoost
:::

::: {.skill-category}
**Data Visualization**  
Tableau • Plotly • Seaborn
:::

::: {.skill-category}
**Big Data & Cloud**  
Apache Spark • Databricks • AWS SageMaker
:::

::: {.skill-category}
**NLP**  
spaCy • Hugging Face • BERT
:::

::: {.skill-category}
**Databases**  
MySQL • PostgreSQL • MongoDB • Snowflake
:::

::: {.skill-category}
**Tools & Methods**  
Docker • Flask • Git • Quarto • A/B Testing • Bayesian Analysis
:::
:::
:::

::: {.about-footer}
*Building data-driven solutions with coffee, creativity and precision* ✨
:::

:::