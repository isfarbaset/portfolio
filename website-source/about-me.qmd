...
for i, item in enumerate(timeline_data):
    ...
    # Title and organization (right side) - balanced positioning
    fig.add_annotation(
        x=0.13,
        y=y_pos + 0.13,
...
document.addEventListener('DOMContentLoaded', function() {
    function adjustTimelineLayout() {
        ...
        const containerWidth = timelineContainer.offsetWidth || window.innerWidth * 0.9;
        const screenWidth = window.innerWidth;
        
        // Calculate responsive values based on container width
        const widthRatio = Math.min(containerWidth / 800, 1); // Normalize to 800px base
        const heightBase = 300 + (100 * widthRatio); // Scale height: 300-400px
        const leftMargin = Math.max(20, 80 * widthRatio); // Scale margins: 20-80px
        const rightMargin = Math.max(60, 180 * widthRatio); // Scale margins: 60-180px
        const xRangeLeft = -0.1 - (0.3 * widthRatio); // Scale x-range: -0.1 to -0.4
        const xRangeRight = 0.4 + (0.6 * widthRatio); // Scale x-range: 0.4 to 1.0
        
        const newLayout = {
            height: Math.round(heightBase),
            margin: {
                l: Math.round(leftMargin),
                r: Math.round(rightMargin),
                t: 20,
                b: 60
            },
            'xaxis.range': [xRangeLeft, xRangeRight],
    ...
    if (window.ResizeObserver) {
        ...
        const container = document.querySelector('.journey-visual');
        if (container) {
